...
// Retrieve all the existing notes for a specific user.
app.get("/getAllNotes", express.json(), async (req, res) => {
    try {
      ...
      jwt.verify(token, "secret-key", async (err, decoded) => {
        ...
        // Find note with given username's ID
        const collection = db.collection(COLLECTIONS.notes);
...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
    try {
      // Basic param checking -- invalide NOTE ID
      ...
      jwt.verify(token, "secret-key", async (err, decoded) => {
        ...
        const result = await collection.findOneAndUpdate(
            { _id: noteId, username: decoded.username },
            { $set: { title, content } },
        ...
        res.json({
          response: "Document properly updated Note updated succesfully.",
          insertedId: result.insertedId,
        }); //200
      });
    } catch (error) {
...
// Ddelete an existing note given the id
